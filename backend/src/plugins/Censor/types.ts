...
export const zCensorConfig = z.strictObject({
  ...
  blocked_regex: z
    ...
    .nullable()
    .default(null),
  // Notify the user when their message is removed
  notify_user: z.boolean().default(true),
  notify_message: z
    .string()
    .max(1000)
    .default("Your message in **{guildName}** was removed. Reason: {reason}"),
  // If set, sends a short notice in this channel instead of DM
  notify_channel: zSnowflake.nullable().default(null),
  notify_channel_message: z
    .string()
    .max(1000)
    .default("{userMention} Your message was removed. Reason: {reason}"),
});

...
export async function applyFiltersToMsg(
  ...
  if (filterZalgo) {
    ...
    if (result) {
      await censorMessage(pluginData, savedMessage, "zalgo detected");
      return true;
    }
  }
  ...
  if (filterInvites) {
    ...
    for (const invite of invites) {
      ...
      if (inviteCodeWhitelist && !inviteCodeWhitelist.includes(invite.code)) {
        await censorMessage(pluginData, savedMessage, `invite code (\`${invite.code}\`) not found in whitelist`);
        return true;
      }

      if (inviteCodeBlacklist && inviteCodeBlacklist.includes(invite.code)) {
        await censorMessage(pluginData, savedMessage, `invite code (\`${invite.code}\`) found in blacklist`);
        return true;
      }
    }
  }
  ...
  if (filterDomains) {
    ...
    for (const thisUrl of urls) {
      if (domainWhitelist && !domainWhitelist.includes(thisUrl.hostname)) {
        await censorMessage(pluginData, savedMessage, `domain (\`${thisUrl.hostname}\`) not found in whitelist`);
        return true;
      }

      if (domainBlacklist && domainBlacklist.includes(thisUrl.hostname)) {
        await censorMessage(pluginData, savedMessage, `domain (\`${thisUrl.hostname}\`) found in blacklist`);
        return true;
      }
    }
  }
  ...
  for (const token of blockedTokens) {
    if (messageContent.toLowerCase().includes(token.toLowerCase())) {
      await censorMessage(pluginData, savedMessage, `blocked token (\`${token}\`) found`);
      return true;
    }
  }
  ...
  for (const word of blockedWords) {
    ...
    if (regex.test(messageContent)) {
      await censorMessage(pluginData, savedMessage, `blocked word (\`${word}\`) found`);
      return true;
    }
  }
  ...
  for (const pattern of config.blocked_regex || []) {
    ...
    if (matches) {
      await censorMessage(pluginData, savedMessage, `blocked regex (\`${regex.source}\`) found`);
      return true;
    }
  }

...
export async function getCaseEmbed(
  ...
  // Detect image URLs from note bodies and display the first one inline in the embed
  const imageUrlRegex = /https:\/\/(cdn|media)\.discordapp\.(com|net)\/attachments\/[^\s]+\.(?:png|jpe?g|gif|webp)/gi;
  for (const note of theCase.notes) {
    const match = note.body.match(imageUrlRegex);
    if (match?.[0]) {
      embed.image = { url: match[0] };
      break;
    }
  }
